...
# POTPUNO BIJELA POZADINA, CRNI I CRVENI DETALJI
st.markdown("""
<style>
...
if not st.session_state.logged_in:
    if st.session_state.auth_mode == "Sign In":
        ...
        if st.button("Create Account & Enter 🚀") and u_reg and len(p_reg) == 4:
            if u_reg in st.session_state.users: st.error("❌ Username already taken!")
            else: st.session_state.users[u_reg] = p_reg; st.session_state.logged_in = True; st.session_state.user = u_reg; st.rerun()
    ...
    if user.upper() == "ADMIN":
        ...
        served = [o for o in st.session_state.orders if o['status'] == "Served" and o['rating'] is not None]
        if served:
        ...
        served_unnotified = [o for o in st.session_state.orders if o['guest'] == user and o['status'] == "Served" and not o.get('notified', False)]
        ...
        u_orders = [o for o in st.session_state.orders if o['guest'] == user]
